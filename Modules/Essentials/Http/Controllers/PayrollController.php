...
class PayrollController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            $payrolls = $this->essentialsUtil->getPayrollQuery($business_id)->addSelect(['from_date', 'to_date']);
            ...
            if (!empty(request()->from_date)) {
                ...
                $payrolls->whereDate('from_date', '>=', $from_date);
            }
            if (!empty(request()->to_date)) {
                $to_date = Carbon::createFromFormat('m/d/Y', request()->to_date);
                $payrolls->whereDate('to_date', '<=', $to_date);
            }
            ...
            return Datatables::of($payrolls)
                ...
                ->editColumn('from_date', '{{!empty($from_date) ? \Carbon::parse($from_date)?->format("m/d/Y") : \Carbon::parse($transaction_date)?->format("m/d/Y")}}')
                ->editColumn('to_date', '{{!empty($to_date) ? \Carbon::parse($to_date)?->format("m/d/Y") : \Carbon::parse($transaction_date)?->lastOfMonth()->format("m/d/Y")}}')
                ...
                ->editColumn(
                    'payment_status',
                    '<a href="{{ action([\App\Http\Controllers\TransactionPaymentController::class, \'show\'], [$id])}}" class="view_payment_modal payment-status-label no-print" data-orig-value="{{$payment_status}}" data-status-name="{{__(\'lang_v1.\' . $payment_status)}}">
                        <span class="label @payment_status($payment_status)">{{__(\'lang_v1.\' . $payment_status)}}</span>
                     </a> <span class="print_section">{{__(\'lang_v1.\' . $payment_status)}}</span>'
                )
                ->removeColumn('id')
        ...
        if (auth()->user()->can('essentials.create_payroll')) {
            $employees = $this->__getEmployeesByLocation($business_id, null, 'month');
        }
    ...
    public function create()
    {
        ...
        $from_date = Carbon::parse($transaction_date);
        $to_date = $from_date->lastOfMonth();
        $payroll_date = Carbon::parse($transaction_date);
        $from_date = request('from_date') ? Carbon::createFromFormat('m/d/Y', request('from_date')) : $from_date;
        $to_date = request('to_date') ? Carbon::createFromFormat('m/d/Y', request('to_date')) : $to_date;

        if (!$payroll_date->isSameMonth($from_date) && !$payroll_date->isSameMonth($to_date)) {
            return redirect()->action([\Modules\Essentials\Http\Controllers\PayrollController::class, 'index'])
                ->with('status', ['success' => true, 'msg' => __('essentials::lang.invalid_date_for_payroll')]);
        }
        ...
        $payrolls = Transaction::where('business_id', $business_id)
                    ...
                    ->whereDate('from_date', $from_date->toDateString())
                    ->whereDate('to_date', $to_date->toDateString())
                    ->get();
        ...
        if (! empty($add_payroll_for)) {
            ...
            $start_date = !empty($from_date) ? $from_date : $transaction_date;
            $end_date = !empty($to_date) ? $to_date : \Carbon::parse($start_date)->lastOfMonth();
            ...
            foreach ($employees as $employee) {
                ...
                if(request()->input('payment_period') == "month"){
                    $payrolls[$employee->id]['essentials_salary'] = $this->moduleUtil->num_uf($employee->essentials_salary)*.5;
                }
            ...
            return view('essentials::payroll.create')
                    ->with(compact('month_name', 'transaction_date', 'year', 'payrolls', 'action', 'location', 'from_date', 'to_date'));
        } else {
            return redirect()->action([\Modules\Essentials\Http\Controllers\PayrollController::class, 'index'])
                ->with('status', [
                    'success' => true,
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($payrolls as $key => $payroll) {
                ...
                //     $payroll['essentials_amount_per_unit_duration'] = $this->moduleUtil->num_uf($payroll['essentials_amount_per_unit_duration'])*.5;
                // }
    ...
    public function show($id): View
    {
        ...
        $start_of_month = $payroll->from_date ? Carbon::parse($payroll->from_date) : Carbon::parse($payroll->transaction_date);
        $end_of_month = $payroll->to_date ? Carbon::parse($payroll->to_date) : Carbon::parse($payroll->transaction_date)->endOfMonth();
        ...
        'total_work_duration', 'location', 'total_days_present', 'start_of_month', 'end_of_month'));
    }
    ...
    public function edit($id): View
    {
        ...
        $transaction_date = \Carbon::parse($payroll->transaction_date);
        $from_date = $transaction_date->firstOfMonth();
        $to_date = $transaction_date->lastOfMonth();
        $month_name = $transaction_date->format('F');
        $year = $transaction_date->format('Y');
        $allowances = ! empty($payroll->essentials_allowances) ? json_decode($payroll->essentials_allowances, true) : [];
        $deductions = ! empty($payroll->essentials_deductions) ? json_decode($payroll->essentials_deductions, true) : [];

        return view('essentials::payroll.edit', compact('payroll', 'month_name', 'allowances', 'deductions', 'year', 'from_date', 'to_date'));
    }
    ...
    public function getEditPayrollGroup($id)
    {
        ...
        foreach ($payroll_group->payrollGroupTransactions as $transaction) {
            ...
            if (empty($transaction_date) && empty($month_name) && empty($year)) {
                ...
                $month_name = $transaction_date->format('F');
                $year = $transaction_date->format('Y');
                $start_date = !empty($transaction->from_date) ? \Carbon::parse($transaction->from_date) : $transaction_date;
                $end_date = !empty($transaction->to_date) ? \Carbon::parse($transaction->to_date) : \Carbon::parse($start_date)->lastOfMonth();
        ...
        $action = 'edit';
        // dd($payrolls);
        return view('essentials::payroll.create', compact('month_name', 'transaction_date', 'year', 'payrolls', 'payroll_group', 'action', 'location', 'from_date', 'to_date'));
    }
    ...
    public function getEmployeesBasedOnLocation(Request $request)
    {
        ...
        try {
            ...
            $employees = $this->__getEmployeesByLocation($business_id, $location_id, $request->get('pay_period'));
    ...
    private function __getEmployeesByLocation($business_id, $location_id = null, $pay_period = null)
    {
        ...
        $query->when(!empty($pay_period), function (Builder $query) use ($pay_period) {
            $query->where('essentials_pay_period', $pay_period);
        });
