                    ...
                    <div class="box-body">
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <h4>Period: {{$from_date->format('m/d/Y')}} to {{$to_date->format('m/d/Y')}}
                                    ({{request('payment_period', $payroll['essentials_duration_unit']??'Month')}})</h4>
                                <small>
                            ...
                            <div class="col-md-4">
                                <?php
                                $payroll_group_name = !empty($payroll_group) ? $payroll_group->name : strip_tags($group_name);
                                if (request('payment_period')) {
                                    $payroll_group_name .= " (" . $from_date->format('m/d/Y') . " to " . $to_date->format('m/d/Y') . ")";
                                }
                                ?>
                                {!! Form::label('payroll_group_name', __( 'essentials::lang.payroll_group_name' ) . ':*') !!}
                                {!! Form::text('payroll_group_name', $payroll_group_name, ['class' => 'form-control', 'placeholder' => __( 'essentials::lang.payroll_group_name' ), 'required']); !!}
                            </div>
                            <div class="col-md-4">
                        ...
                        <table class="table" id="payroll_table">
                            ...
                            @foreach($payrolls as $employee => $payroll)
                                @php
                                    $days_in_the_month = Carbon::parse($transaction_date)->daysInMonth;
                                    $total_work_days = $days_in_the_month;
                                    if ($action != 'edit') {
                                    ...
                                    $amount_per_day = round($total / $total_work_days, 2);
                                    $total_work_days = $from_date->diffInDays($to_date)+1; 
                                    if (strtolower($payroll['essentials_pay_period']??$payroll['essentials_duration_unit']) == 'week') {
                                        $total_work_duration = $from_date->diffInWeeks(Carbon::parse($to_date)->addDay());
                                    }
                                    if (strtolower($payroll['essentials_pay_period']??$payroll['essentials_duration_unit']) == 'day') {
                                        $total_work_duration = $from_date->diffInDays(Carbon::parse($to_date)->addDay());
                                    }
                                ...
                                <tr data-id="{{$employee}}">
                                    <input type="hidden" name="payrolls[{{$employee}}][expense_for]"
                                           value="{{$employee}}">
                                    <input type="hidden" name="payrolls[{{$employee}}][from_date]"
                                           value="{{$from_date->toDateString()}}">
                                    <input type="hidden" name="payrolls[{{$employee}}][to_date]"
                                           value="{{$to_date->toDateString()}}">
                                    @if($action == 'edit')
                                    ...
                                    <td>
                                        ...
                                        <br><br>
                                        <b>{{__('essentials::lang.duration_unit')}}:</b>
                                        {{$duration_unit}}
                                    </td>
                                    <td>
                                        <div class='mb-1'>
                                            {!! Form::label('essentials_duration_day_'.$employee, __( 'essentials::lang.attendance' ) . ':*') !!}
                                            {!! Form::text('payrolls['.$employee.'][essentials_duration_day]', $payroll['total_days_worked'], ['class' => 'form-control input_number essentials_duration_day', 'placeholder' => __( 'essentials::lang.total_work_duration' ), 'required', 'data-id' => $employee, 'id' => 'essentials_duration_day_'.$employee]); !!}
                                        </div>
                                        ...
                                        <div class='mb-1'>
                                            ...
                                            {!! Form::text('payrolls['.$employee.'][essentials_duration_unit]', $duration_unit, ['class' => 'form-control', 'placeholder' => __( 'essentials::lang.duration_unit' ), 'data-id' => $employee, 'id' => 'essentials_duration_unit_'.$employee, 'readonly']); !!}
                                        </div>
                                        ...
                                        <div class='mb-1' id="essentials_amount_per_day_div_{{$employee}}">
                                            {!! Form::label('essentials_amount_per_day_'.$employee, __( 'essentials::lang.amount_per_day' ) . ':*') !!}
                                            {!! Form::text('payrolls['.$employee.'][essentials_amount_per_day]', $amount_per_day, ['class' => 'form-control input_number essentials_amount_per_day', 'placeholder' => __( 'essentials::lang.amount_per_day' ), 'required', 'data-id' => $employee, 'id' => 'essentials_amount_per_day_'.$employee]); !!}
                                        </div>
                                        ...
                                        @component('components.widget')
                                            <table class="table table-condenced allowance_table"
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <span id="total_allowances_{{$employee}}"
