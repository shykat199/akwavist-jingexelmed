...
$(document).ready( function () {
	...
	$(document).on('change', '.total', function() {
        let total_duration = __read_number($(this).closest('td').find('input.essentials_duration_day'));
        let total = __read_number($(this));
        let amount_per_day = total / total_duration;
        __write_number($(this).closest('td').find('input.essentials_amount_per_day'), amount_per_day, false, 2);
        calculateTotal($(this).data('id'));
        calculateTotalGrossAmount();
    });

    $(document).on('change', '.essentials_duration_day, .essentials_amount_per_day, input.allowance, input.deduction, input.percent', function() {
        let id = $(this).data('id');
    ...
    function calculateTotal (id) {
    	...
    	let total_duration = __read_number($("input#essentials_duration_day_"+id));
	    let amount_per_day = __read_number($("input#essentials_amount_per_day_"+id));
        let essentials_duration_unit = $('input#essentials_duration_unit_'+id).val();
        let total = total_duration * amount_per_day;
        if (essentials_duration_unit == 'Month') {
            total = $("input#essentials_amount_per_unit_duration_"+id).val();
            $("#essentials_amount_per_day_div_"+id).hide();
        }
        ...
        var gross_amount = parseFloat(total) + parseFloat(total_allowance) - parseFloat(total_deduction);
