...
$(document).ready(function() {
    ...
    $(document).on('change', '.input_quantity', function() {
        var tempp = $(this).parent().parent().find('.default_purchase_price_p');
        if (!tempp.data('original')) {
        tempp.data('original', tempp.val());
    }
    var originalPrice = parseFloat(tempp.data('original'));
    var quantity = parseFloat($(this).val());

    if (!isNaN(originalPrice) && !isNaN(quantity)) {
        tempp.val(originalPrice * quantity);
    }
    var sum = 0;
    $('.default_purchase_price_p').each(function() {
        var value;
    ...
    $('.total_cost_p').text(sum.toFixed(2));
    });
